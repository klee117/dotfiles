...
# Start in Linux home directory when shell opens from Windows
[[ "$PWD" == /mnt/c/* ]] && cd ~
...
alias oc='tmux-profile opencode'
alias fd=fdfind

export EDITOR=nvim

# fdo: fzf-pick a file or folder under the current dir; cd into folders, open files in nvim
fdo() {
  local sel _fd
  if command -v fdfind &>/dev/null; then _fd=fdfind
  elif command -v fd &>/dev/null; then _fd=fd
  fi
  if [[ -n "$_fd" ]]; then
    sel=$("$_fd" --hidden --exclude .git . | fzf --prompt='fdo> ' --height=40% --reverse) || return
  else
    sel=$(find . -not \( -name .git -prune \) | fzf --prompt='fdo> ' --height=40% --reverse) || return
  fi
  [[ -z "$sel" ]] && return
  if [[ -d "$sel" ]]; then
    cd "$sel" || return
